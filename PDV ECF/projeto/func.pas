...
function ArredondaPDV( valor:currency; decimais:integer; tipo : string = '') : currency;
...
begin
  ...
  if paramGe38 = '2' then
    begin
      ...
      Result.quant := ArredondaPDV((quanti /100) / Result.preco, 2, arredon);
      quanti := Result.quant; // quanti tem  a quantidade aprox

      while true do
        begin
          _ptot := ArredondaPDV(quanti * Result.preco, 2, arredon);//valor aprox
    ...
    begin
      ...
      Result.precoTemp := ArredondaPDV(Result.quant * Result.preco, 2, arredon);
    end;
end;
...
begin
  ...
  if arq.Values['controlePorta'] = 'S' then ecf.ControlePorta := true
    else ecf.ControlePorta := false;

  if StrToIntDef(arq.Values['interComandos'], 0) > 100 then ecf.IntervaloAposComando := StrToIntDef(arq.Values['interComandos'], 0);
