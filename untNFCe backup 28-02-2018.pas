...
begin
  ...
  if trim(query1.FieldByName('ESTADO').AsString) = 'C' then DANFE_Rave.NFeCancelada := true;
...
{  if ((now >= StrToDate('03/07/2017')) and (ACBrNFe.Configuracoes.WebServices.Ambiente = taHomologacao)) then begin
  ...
  //a versao 3.10 vai parar de funcionar 02/04/2018, vou mudar o sistema pra esse dia
  if ((now >= StrToDate('01/03/2018')) and (ACBrNFe.Configuracoes.WebServices.Ambiente = taProducao)) then begin
    versao := '4.00';
    ACBrNFe.Configuracoes.Geral.VersaoDF := ve400;
  end;     }
...
begin
  ...
  if ((length(strnum(fone)) < 6) or (length(strnum(fone)) > 14)) then begin
    fone := '';
  end;
       ...
       if (cSta in [100, 150]) then
         ...
         begin
           ...
           ShowMessage('NF-e '+ ' Houve uma falha na Validação!'+#13+#10+#13+#10+
                  'Favor Corrigir: ' + ACBrNFe.WebServices.Enviar.xMotivo);//+qry.FieldByName('RECIBO_DESCSTATUS').AsString);
           Result := false;
           LimpaVariaveis;
           exit;
         end;
  ...
  try
    ...
    On e:Exception do
      begin
        ...
        if ((ACBrNFe.WebServices.Enviar.cStat = 778) or (false)) then begin
          validaNCM_NaNFCe('');
          exit;
        end;
        ...
        if ACBrNFe.WebServices.Enviar.cStat = 205 then begin
          query1.Close;
        ...
        if ACBrNFe.WebServices.Enviar.cStat = 206 then begin
          query1.Close;
        ...
        //
        if ACBrNFe.WebServices.Enviar.cStat <> 100 then begin
          estado := estado + #13 + ACBrNFe.WebServices.Enviar.xMotivo + '| cStat: ' + IntToStr(ACBrNFe.WebServices.Enviar.cStat);
          gravaERRO_LOG1('', 'erro Linha 3276 ' + estado + FormatDateTime('dd/mm/yyyy', now) + FormatDateTime('hh:mm:ss', now), 'trataDuplicidade1');
        end;

        if (Contido('Duplicidade de NF-e', e.Message)) or (ACBrNFe.WebServices.Enviar.cStat = 539) or (ACBrNFe.WebServices.Enviar.cStat = 204) THEN
          begin
            try
              estado := '|ii| '+e.Message + #13 + ACBrNFe.WebServices.Enviar.xMotivo + '| cStat: ' + IntToStr(ACBrNFe.WebServices.Enviar.cStat);
...
begin
 ...
 carregaConfigsNFCe;
 query1.Close;
 query1.SQL.Text := 'select chave from nfce where cast(substring(chave from 26 for 9) as integer) = :nota';
...
begin
  ...
  if ACBrNFe.WebServices.Enviar.cStat <= 200 then
    begin
      ...
      stats := 'OK';
      venda.chave := ssChave;
      if ACBrNFe.WebServices.Enviar.cStat = 100 then venda.adic := '';
      insereNotaBD(venda);
    end;
end;
...
begin
  ...
  for i := 0 to cont do
    begin
      ...
      '<uCom>' + item.unid + '</uCom><qCom>' + Format_num(0, FormatCurr('0.0000',item.quant)) + '</qCom><vUnCom>' +
      Format_num(0, FormatCurr('0.000000',item.p_venda)) + '</vUnCom><vProd>' + FORMAT_NUM(item.total) + '</vProd><cEANTrib></cEANTrib>' +
      '<uTrib>' + item.unid + '</uTrib><qTrib>' + Format_num(0, FormatCurr('0.0000',item.quant)) + '</qTrib><vUnTrib>' +
      Format_num(0, FormatCurr('0.000000',item.p_venda)) + '</vUnTrib>'+ '<vFrete>'+ Format_num(item.Vlr_Frete) +'</vFrete>' + IfThen(item.Desconto = 0,'<vDesc>0.00</vDesc>','<vDesc>' + FORMAT_NUM(item.Desconto) + '</vDesc>') +
...
begin
  ...
  if exterior then begin
    ...
    if length(query1.fieldbyname('ende').AsString) < 5 then begin
      ShowMessage('Destinatário: Endereço ' + query1.fieldbyname('ende').AsString + ' Muito Curto!');
      exit;
    end;

    if length(query1.fieldbyname('bairro').AsString) < 2 then begin
     ShowMessage('Destinatário: Bairro ' + query1.fieldbyname('bairro').AsString + ' Muito Curto!');
     exit;
    end;
...
begin
  ...
  minhaThreadTeste := TIdThreadComponent.Create(Application);
  //minhaThreadTeste.OnRun := IdThreadComponent2Run;
  minhaThreadTeste.Loop  := true;
  minhaThreadTeste.Start;
...
begin
  Result := '';
  if not FileExists(ExtractFileDir(ParamStr(0)) + '\IBPT.csv') then exit;

  arq := TStringList.Create;
  arq.LoadFromFile(ExtractFileDir(ParamStr(0)) + '\IBPT.csv');
  if arq.Count > 10 then begin
    try
      if StrToInt(ContaChar(arq[5], ';')) > 4 then begin
        val := ';' + arq[5] + ';';
