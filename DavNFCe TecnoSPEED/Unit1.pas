...
uses
  ...
  ACBrNFeDANFeESCPOS, ACBrDANFCeFortesFr, Vcl.Menus, RxShell,
  spdNFeDataSets, spdNFCeDataSets, spdNFCe, spdNFe;
...
type
  TForm1 = class(TForm)
    ...
    Button2: TButton;
    ComboBox1: TComboBox;
    spdNFeDataSets1: TspdNFeDataSets;
    spdNFCe1: TspdNFCe;
  ...
  private
    ...
    procedure copiaVendas(nota: integer; var notaNova: integer; chave: String;
      Cancelado: integer = 0);
...
begin
  ...
  QueryControlProd.SQL.Text :=
    'select nota, chave, data from nfce where ((adic = ''OFF'') and (substring(chave from 23 for 3) = :serie)) ';
  ...
  begin
    ...
    try
      {envi := EnviarCupomEletronico2(QueryControlProd.fieldbyname('nota')
        .AsString, QueryControlProd.fieldbyname('chave').AsString, esta, false,
        true, false);}
    except
      on e: exception do
      begin
...
begin
  ...
  try
    ...
    try
      if contAtualizacao > 40 then
      //if contAtualizacao > 1 then
      begin
        try
          sincronizaEstoque;
        except
      ...
      try
        ...
        on e: exception do
        begin
          RichEdit1.Lines.Add('ERRO 356: ' + e.Message);
        end;
      end;
    except
      on e: exception do
...
begin
  spdNFe1.ListarCertificados(ComboBox1.Items);
...
begin
  ...
  {spdNFe1.UF := 'RR';
  spdNFe1.DiretorioEsquemas := ExtractFileDir(ParamStr(0)) + '\Esquemas';
  spdNFe1.SenhaCertificado := '210305';}


  {spdNFe1.NomeCertificado.Text := ComboBox1.Items[ComboBox1.ItemIndex];
  spdNFe1.ArquivoServidoresHom  := ExtractFileDir(ParamStr(0)) + '\nfeServidoresHom.ini';
  spdNFe1.ArquivoServidoresProd := ExtractFileDir(ParamStr(0)) + '\nfeServidoresProd.ini';
  spdNFe1.DiretorioTemplates := ExtractFileDir(ParamStr(0)) + '\templates';}

  spdNFCe1.UF  := 'RR';
  spdNFCe1.CNPJ := '10467211000192';
  spdNFCe1.DanfceSettings.IdTokenNFCe := '000001';
  spdNFCe1.DanfceSettings.TokenNFCe   := '1003794059AFF1FBE050010AB5C06754';

  spdNFCe1.DiretorioEsquemas := ExtractFileDir(ParamStr(0)) + '\Esquemas';
  spdNFCe1.SenhaCertificado := '210305';
  spdNFCe1.NomeCertificado.Text := ComboBox1.Items[ComboBox1.ItemIndex];
  spdNFCe1.ArquivoServidoresHom  := ExtractFileDir(ParamStr(0)) + '\nfceServidoresHom.ini';
  spdNFCe1.ArquivoServidoresProd := ExtractFileDir(ParamStr(0)) + '\nfceServidoresProd.ini';
  spdNFCe1.DiretorioTemplates := ExtractFileDir(ParamStr(0)) + '\templates';

  spdNFCe1.SaveConfig(ExtractFileDir(ParamStr(0)) + '\configTecno.ini');
  //RichEdit1.Lines.Text := spdNFCe1.StatusDoServico;
  //exit;

  //RichEdit1.Lines.Add(spdNFe1.ConsultarNF(InputBox('','','')));
  op := TOpenDialog.Create(self);
  op.Execute;
  arq := TStringList.Create;
  arq.LoadFromFile(op.FileName);

  arq.Text := spdNFCe1.AssinarNota(arq.Text);
  arq.savetofile('D:\Teste.txt');
  //spdNFCeDataSets1.Incluir;
  //spdNFCeDataSets1.A.Create(self);
   RichEdit1.Lines.Text := spdNFCe1.EnviarNFSincrono('0', arq.Text);
  ///RichEdit1.Lines.Add(spdNFe1.StatusDoServico);
end;
...
begin
  ...
  if ((pastaControlW_Servidor = 'XX') or
    ((not bdPronto) and (pastaControlW_Servidor <> ''))) then
  begin
    IBQuery1.Close;
    IBQuery1.SQL.Text :=
      'select chave, nota from nfce where (adic <> ''OFF'')  and exportado = 0';
    IBQuery1.Open;
    IBQuery1.FetchAll;
    ...
    begin
      ...
      if FileExists(ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\' +
  ...
  begin
    IBQuery1.Close;
    IBQuery1.SQL.Text :=
      'select chave, nota from nfce where ((adic = '''') or (adic = ''CANC'')) and exportado = 0';
    IBQuery1.Open;
    IBQuery1.FetchAll;
...
procedure TForm1.copiaVendas(nota: integer; var notaNova: integer;
  chave: String; Cancelado: integer = 0);
begin
  IBQuery2.Close;
  ...
  begin
    IBQueryServer1.Close;
    IBQueryServer1.SQL.Text :=
      'insert into item_venda(nota, cod, quant, p_venda, total, aliquota, data, unid)'
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        arq.Free;
        ACBrNFe1.NotasFiscais[0].GravarXML(chave + '-nfe.xml',
          ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\');
      end;
    ...
    try
      if not DirectoryExists(pastaControlW_Servidor + 'NFCe\EMIT\') then
        ForceDirectories(pastaControlW_Servidor + 'NFCe\EMIT\');

      if FileExists(ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\' + chave +
        '-nfe.xml') then
      begin
        Result := CopyFile(pchar(ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\' +
          chave + '-nfe.xml'), pchar(pastaControlW_Servidor + 'NFCe\EMIT\' +
          chave + '-nfe.xml'), true);
      end;
    except
      on e: exception do
    ...
    begin
      RichEdit1.Lines.Add('XML ' + chave + ' Copiado para ' +
        pastaControlW_Servidor + 'NFCe\EMIT\');
      RichEdit1.Lines.Add('XML EXPORTTADO' + chave + ' !');
    end
    else
    begin
      RichEdit1.Lines.Add('ERRO NA COPIA XML ' + chave);
      RichEdit1.Lines.Add('DE ' + ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\' +
        chave + '-nfe.xml' + '  PARA ' + pastaControlW_Servidor + 'NFCe\EMIT\' +
        chave + '-nfe.xml');
    end;
    arq.Free;
    exit;
  ...
  try
    ...
    try
      ...
      arq.LoadFromFile(ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\' +
        IBQuery2.fieldbyname('chave').AsString + '-nfe.xml');
      if Le_Nodo('cStat', arq.Text) = '' then
      begin
        ACBrNFe1.NotasFiscais.LoadFromFile(ExtractFileDir(ParamStr(0)) +
          '\NFCe\EMIT\' + IBQuery2.fieldbyname('chave').AsString + '-nfe.xml');
        // ACBrNFe1.Consultar;
      end;
    except
      on e: exception do
    ...
    try
      if not DirectoryExists(pastaControlW_Servidor + 'NFCe\EMIT\') then
        ForceDirectories(pastaControlW_Servidor + 'NFCe\EMIT\');
      CopyFile(pchar(ExtractFileDir(ParamStr(0)) + '\NFCe\EMIT\' +
        IBQuery2.fieldbyname('chave').AsString + '-nfe.xml'),
        pchar(pastaControlW_Servidor + 'NFCe\EMIT\' +
      ...
      begin
        if ((pastaControlW_Servidor <> 'XX') and (pastaControlW_Servidor <> '')) then begin
          try
            ...
            CopyFile(pchar(ExtractFileDir(ParamStr(0)) + '\NFe\EMIT\' +
