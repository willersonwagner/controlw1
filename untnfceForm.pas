...
begin
  try
    ...
    begin
      ...
      MessageDlg('Erro na Impress�o: ' + #13 + e.Message + #13, mtError,
        [mbOK], 1);
    end;
  end;
end;
...
begin
  ...
  // busca as chaves da mesma numera��o e a mesma serie para excluir
  ...
  // deleta as chaves que estiverem a mesma numera��o e mesma serie
  query1.Close;
  query1.SQL.text :=
...
{$ENDIF}
    ...
    begin
      ...
      AcbrEmail.SetSSL := EmailSSL; // SSL - Conexão Segura
      ...
      AcbrEmail.UseThread := false; // Aguarda Envio do Email(não usa thread)
    end;
...
begin
    ...
    try
      ...
      begin
        ...
        Status := 'vali';
        MessageDlg('Falha na Valida��o da Nota11: ' + #13 + e.Message + #13,
          mtError, [mbOK], 1);
        Result := false;
        exit;
      end;
...
begin
  ...
  begin
    query1.Close;
    if not dav then
      ShowMessage('NFCe n�o encontrada');
    exit;
  end;
  ...
  begin
    Result := false;
    richED.Lines.Add('NFCe N�o Encontrada: ' + buscaPastaNFCe(CHAVENF) + CHAVENF
      + '-nfe.xml');
    exit;
  end
  else
  begin
    ...
    arq.LoadFromFile(buscaPastaNFCe(CHAVENF) + CHAVENF + '-nfe.xml');
    if Contido('"3.10"', arq.text) then
    begin
    ...
    if Contido('<detPag>', arq.text) = false then
    begin
      erro2 := Le_Nodo('pag', arq.text);
      erro2 := '<pag><detPag>' + erro2 + '</detPag></pag>';
  ...
  try
    ...
    begin
      ...
      gravaERRO_LOG1('', 'Troca de Chave Por Erro de Envio: ' + #13#10 +
        ...
        FormatDateTime('hh:mm:ss', now), 'Nova Cria��o de XML');
    ...
    try
      try
        ...
        begin
          ...
          if Contido('(5)-Requisi��o n�o', ERRO_dados) and (csta <> 100) then
            exit;
          ...
          begin
            ...
            { if erro12002 > 3 then begin
              ...
              estado := estado + #13 + #13 + 'O davNFCe Ser� Reiniciado!';
              end; }
            exit;
          end;
          ...
          begin
            ...
            estado := e.Message + #13 + 'NFCe ' + CHAVENF +
              ' j� est� marcada no BD como Cancelada!';
            exit;
          end;
      ...
      begin
        if dav = false then
        begin
          ShowMessage('NF-e ' + ' Houve uma falha na Valida��o!' + #13 + #10 +
      ...
      try
        ...
        begin
          ...
          // MessageDlg('Verifique se o Acrobat est� instalado corretamente.', mtInformation, [mbok],0 );
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      query1.Close;
      ShowMessage('NFCe n�o encontrada1');
      exit;
    end;
  ...
  begin
    query1.Close;
    ShowMessage('NFCe n�o encontrada2');
    exit;
  end;
...
begin
  ...
  begin
    ShowMessage('Ocorreu um Erro no Cancelamento:' + #13 + tmp +
      ...
      #13 + #13 + 'Esta Nota j� pode ter Sido Cancelada', ''));
  end;
end;
  ...
  begin
    ...
    begin
      try
        ...
        on e: Exception do
        begin
          MessageDlg('Ocorreu um Erro Na Impress�o: ' + e.Message, mtError,
            [mbOK], 1);
        end;
      end;
    end
...
begin
  ...
  begin
    query1.Close;
    ShowMessage('NFCe de N�mero ' + numeroNota + ' N�o Encontrado.');
    exit;
  end;
...
begin
  ...
  if (POS('Refeicao', formaPagto) > 0) or (POS('REFEICAO', formaPagto) > 0) or
    (POS('refei��o', formaPagto) > 0) or (POS('REFEI��O', formaPagto) > 0) or
    (POS('Refei��o', formaPagto) > 0) then
  ...
  if (POS('Credito', formaPagto) > 0) or (POS('Cr�dito', formaPagto) > 0) or
    (POS('credito', formaPagto) > 0) or (POS('cr�dito', formaPagto) > 0) or
    (POS('CREDITO', formaPagto) > 0) or (POS('CR�DITO', formaPagto) > 0) then
    Result := fpCartaoCredito
  else
    Result := fpDinheiro;

  if (POS('Debito', formaPagto) > 0) or (POS('D�bito', formaPagto) > 0) or
    (POS('debito', formaPagto) > 0) or (POS('d�bito', formaPagto) > 0) or
    (POS('DEBITO', formaPagto) > 0) or (POS('d�bito', formaPagto) > 0) then
  ...
  begin
    ...
    begin
      ...
      try
        ...
        ShowMessage('Arquivo NFC-e Inv�lido');
        exit;
      end;
    end;
...
begin
  ...
  if not ACBrNFe.WebServices.EnvEvento.EventoRetorno.retEvento[0]
    .RetInfEvento.cstat > 200 then
  begin
    MessageDlg('Evento - N�o foi poss�vel fazer o Download do XML:' + #13 +
  ...
  begin
    MessageDlg('NFe - N�o foi poss�vel fazer o Download do XML:' + #13 + #13 +
...
begin
  ...
  begin
    dadosDest.Values['cod'] := '';
    Result := Result + 'CPF/CNPJ Inv�lidos.';
  end;
  tipo := trim(tipo);
  ...
  IF NOT ok then
  begin
    Result := Result + ' CPF/CNPJ do Destinat�rio Inv�lido ' + #13;
    invalido := invalido + 1;
  end;

  if CAMPO_VAZIO(dadosDest.Values['ende']) = 1 then
  begin
    Result := Result + 'Endere�o Inv�lido ' + #13;
    invalido := invalido + 1;
  end;

  if CAMPO_VAZIO(dadosDest.Values['bairro']) = 1 then
  begin
    Result := Result + 'Bairro Inv�lido ' + #13;
    invalido := invalido + 1;
  end;

  if CAMPO_VAZIO(dadosDest.Values['cod_mun']) = 1 then
  begin
    Result := Result + 'C�digo do Municipio Inv�lido ' + #13;
    invalido := invalido + 1;
  end;

  if CAMPO_VAZIO(dadosDest.Values['cid']) = 1 then
  begin
    Result := Result + 'Nome do Municipio Inv�lido ' + #13;
    invalido := invalido + 1;
  end;

  if CAMPO_VAZIO(dadosDest.Values['est']) = 1 then
  begin
    Result := Result + 'Estado Inv�lido ' + #13;
    invalido := invalido + 1;
  end;

  if CAMPO_VAZIO(dadosDest.Values['cep']) = 1 then
  begin
    Result := Result + 'Cep Inv�lido ' + #13;
    invalido := invalido + 1;
  end;
...
begin
  ...
  // asume o valor padr�o como valor de retorno
  Result := default;
  // se config.dat n�o encontrado, cria e retorna o valor default
  ...
  // se o valor do parametro � v�lido, pega esse valor
...
begin
  ...
  IF NOT ok then
  begin
    ERRO_dados := ' CPF/CNPJ do Destinat�rio Inv�lido ' + #13;
    invalido := invalido + 1;
  end;
...
begin
  if msg then
  begin
    MessageDlg('Ocorreu Um Erro de Duplicidade e o Sistem ir� recuperar a NFe!',
      mtWarning, [mbOK], 1);
  end;
...
begin
  ...
  begin
    ...
    gravaERRO_LOG1('', buscaPastaNFCe(chavb.chave) + chavb.chave +
      '-nfe.xml  N�o Existe!', 'Nova Cria��o de XML');
    // exit;
  end;
...
begin
  ...
  begin
    MessageDlg('Tabela do IBPT N�o Foi Encontrada, Favor Contate o Suporte!',
...
begin
  ...
  begin
    MessageDlg('O Sistema s� pode Emitir NFCe quando As Notas Forem Enviadas: '
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Destinat�rio: Informa��o de Identidade ' + CNPJ +
        ' Inv�lido!');
      exit;
    end;
    ...
    begin
      ShowMessage('Destinat�rio: Endere�o ' + query1.fieldbyname('ende')
    ...
    begin
      ShowMessage('Destinat�rio: Bairro ' + query1.fieldbyname('bairro')
        .AsString + ' Muito Curto!');
      exit;
    end;
  ...
  if query1.IsEmpty then
  begin
    ShowMessage('Cliente ' + strnum(cod) + ' N�o Existe!');
    query1.Close;
    exit;
  end;
  ...
  begin
    ...
    begin
      acc := acc + 'Destinat�rio: CPF ' + CNPJ + ' Inv�lido!' + #13;
    end;
  end
  else
  begin
    ...
    begin
      acc := acc + 'Destinat�rio: CNPJ ' + CNPJ + ' Inv�lido!' + #13;
    end;
  end;

  if length(query1.fieldbyname('ende').AsString) < 5 then
  begin
    acc := acc + 'Destinat�rio: Endere�o ' + query1.fieldbyname('ende').AsString
      + ' Muito Curto!' + #13;
  end;
  ...
  begin
    acc := acc + 'Destinat�rio: C�digo do Munic�pio ' +
      query1.fieldbyname('cod_mun').AsString + ' Inv�lido!' + #13;
  end;

  if query1.fieldbyname('est').AsString = '' then
  begin
    acc := acc + 'Destinat�rio: Estado N�o Preenchido!' + #13;
  end;

  if query1.fieldbyname('cid').AsString = '' then
  begin
    acc := acc + 'Destinat�rio: Cidade N�o Preenchida!' + #13;
  end;

  if query1.fieldbyname('cep').AsString = '' then
  begin
    acc := acc + 'Destinat�rio: CEP N�o Preenchido!' + #13;
  end;

  if length(query1.fieldbyname('bairro').AsString) < 2 then
  begin
    acc := acc + 'Destinat�rio: Bairro Inv�lido!' + #13;
  end;

  if acc <> '' then
  begin
    ShowMessage('Dados do Destinat�rio Incompletos: ' + #13 + #13 + acc);
    exit;
  end;
...
begin
  ...
  begin
    ...
    begin // se acabou o tempo de espera
      ERRO_dados := '(5)-Requisi��o n�o enviada';
...
begin
  ...
  begin
    ...
    begin
      if exception1 then
      begin
        Exception.Create('(1)O Certificado Digital Est� Vencido!' + #13 +
          'Entre' + ' em contato com o suporte para atualiza��o do certificado'
          + #13 + 'Vencimento: ' + FormatDateTime('dd/mm/yyyy',
          ACBrNFe.SSL.CertDataVenc));
        exit;
      end;

      MessageDlg('(1)O Certificado Digital Est� Vencido!' + #13 + 'Entre' +
        ' em contato com o suporte para atualiza��o do certificado' + #13 +
...
begin
  ...
  try
    ...
    try
      // o nome 'arquivo' � obrigat�rio, n�o se pode mudar se n�o mudar no arquivo php tbm.
