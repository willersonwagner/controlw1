  ...
  begin
    if key = 116 then
      begin
        ...
        data := funcoes.dialogo('data',0,'1234567890'+#8,50,true,'',Application.Title,'Qual a Previs�o para Pagamento?',data);
        if data = '*' then exit;
...
begin
  ...
  if verificaValorNegativo then
    begin
      WWMessage('Foi detectado uma conta negativa, primeiro d� baixa desta conta utilizando ENTER para poder baixar utilizar esta fun��o', mtWarning,[mbok], HexToTColor('FFD700'),true,false, HexToTColor('B22222'));
      exit;
    end;
  ...
  if ((StrToCurr(valorBaixado) <= 0) or (StrToCurr(valorBaixado) > totGeral)) then begin
    MessageDlg('Valor Inv�lido: ' + valorBaixado, mtInformation, [mbOK], 1);
    exit;
  end;
...
begin
  ...
  valorBaixado := funcoes.ConverteNumerico(funcoes.dialogo('numero',0,'1234567890,.'+#8,2,false,'',Application.Title,'O Valor � R$ '+FormatCurr('#,###,##0.00',DBGrid1.DataSource.DataSet.FieldByName('valor').AsCurrency)+' .Qual o Valor a Ser Baixado?',FormatCurr('#,###,###0.00',DBGrid1.DataSource.DataSet.FieldByName('valor').AsCurrency)));
  ...
  //se for maior d� uma mensagem que excedeu o valor da conta
