...
procedure TForm1.assinaNotas();
var
  arq : TStringList;
  i   : integer;
begin
  if FileExists(ExtractFileDir(ParamStr(0)) + '\notas.ass') = false then exit;

  arq := TStringList.Create;
  arq.LoadFromFile(ExtractFileDir(ParamStr(0)) + '\notas.ass');

  for i := 0 to arq.Count -1 do begin
    if FileExists(buscaPastaNFCe(arq[i]) + arq[i] + '-nfe.xml') then begin
      ACBrNFe1.NotasFiscais.Clear;
      ACBrNFe1.NotasFiscais.LoadFromFile(buscaPastaNFCe(arq[i]) + arq[i] + '-nfe.xml');
      acbrnfe1.NotasFiscais.Assinar;
      if acbrnfe1.NotasFiscais[0].GravarXML(arq[i] + '-nfe.xml', buscaPastaNFCe(arq[i])) then RichEdit1.Lines.Add('XML Assinado: ' + arq[i])
        else RichEdit1.Lines.Add('Erro em assinar XML: ' + arq[i]);
    end
    else RichEdit1.Lines.Add('XML nao Encontrado: ' + buscaPastaNFCe(arq[i]) + arq[i] + '-nfe.xml');
  end;
