...
begin
  ...
  {P4InfoVarejo_Formulario     := TStringList.create;
  P4InfoVarejo_Componente     := TStringList.create;
  P4InfoVarejo_Ativar         := TStringList.create;
  P4InfoVarejo_visivel        := TStringList.Create;
  P4InfoVarejo_leitura        := TStringList.Create;
  P4InfoVarejo_ChaveAplicacao := '\SOFTWARE\P4INFORMATICA\CORPORE';
  try
    repeat
      resposta := LoginBancoDados(P4InfoVarejo_ChaveAplicacao, dtmmain.conMain);
      if resposta = 4 then
        configurar(P4InfoVarejo_ChaveSeguranca)
      else if resposta = 6 then
        configurar(P4InfoVarejo_ChaveAplicacao);
  ...
  //dtmMain.conmain.Connected := true;
  AD_cdserie := '1';
  Application.Initialize;
  Application.Title := 'AUTOCOM ECF V 1.0.0';
