...
begin
  ...
  form40.ListBox1.Items.Add('38=Qual a Informação contida na Etiqueta da balança (1-Quantidade 2-Preço Total) ?');
...
begin
  ...
  grupo:='';
  grupo := funcoes.dialogo('generico',0,'1234567890'+#8,50,false,'',Application.Title,'Qual o Grupo?','');
  if not funcoes.Contido('*',grupo) then his := funcoes.dialogo('generico',0,'1234567890'+#8,50,false,'',Application.Title,'Informe o Cód. do Histórico?','');
  ...
  if not funcoes.Contido('*',grupo+ini+fim+saldo+his) then  resumo := funcoes.dialogo('generico',0,'SN',20,false,'S',Application.Title,'Imprimir Resumo por Histórico?','N');
...
if funcoes.lista1 = '0' then
 begin
   ...
   form36.ListBox1.Items.Add('2-Pedir Confirmação para o preço de cada produto na venda (S-Reais P-Porcent. X-Ambos) ?');
...
begin
  ...
  funcoes.informacao(0, ultimo,'AGUARDE CALCULANDO ESTOQUE MÍNIMO... ',true,false,2);
  while not dm.IBQuery3.Eof do
    begin
      funcoes.informacao(dm.IBQuery3.RecNo, ultimo, 'AGUARDE CALCULANDO ESTOQUE MÍNIMO... ',false,false,2);
  ...
  funcoes.informacao(dm.IBQuery3.RecNo, ultimo, 'AGUARDE CALCULANDO ESTOQUE MÃNIMO... ',false,TRUE,5);
...
begin
  ...
  base := funcoes.dialogo('not', 0, 'SNA', 20, true, 'S', Application.Title, 'Calcular com base no Preço da Venda (S-Sim, N-Não, A-Atacado) ?', '');
  if base = '*' then exit;

  estoqueZero := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Imprimir Itens Com Estoque Zero (S-Sim, N-Não) ?', '');
  ...
  ordem := funcoes.dialogo('not', 0, '123', 50, true, 'S', Application.Title, 'Qual a Ordem do Relatório (1-Alfabética, 2-Fornecedor, 3-Fabricante) ?', '');
  if ordem = '*' then exit;
...
{  if selecao = '' then
  ...
  arredond := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Arrendondar Quantidades para Inteiro (S-Sim, N-Não)?', '');
  if arredond = '*' then exit;

  impri := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Enviar para Impressora (S-Sim, N-Não) ?', '');
  if impri = '*' then exit;
...
begin
  unid := '';
  unid := funcoes.dialogo('generico',0,'ABCDEFGHIJLMNOPKXYZWQRSTUVXZ',50,false,'S',Application.Title,'Confirme o Drive para Gerar a Exportação?', ConfParamGerais[33]);
  if unid = '*' then exit;
  unid := unid + ':\';
...
begin
  ...
  sim := funcoes.dialogo('generico',0,'SN',30,false,'S',Application.Title,'Imprimir Contas (1-Todas, 2-Pagas, 3-Não Pagas) ?','1');
  if sim = '*' then exit;
