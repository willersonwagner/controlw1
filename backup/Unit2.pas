...
begin
  ...
  while not dm.IBselect.Eof do begin
    funcoes.informacao(dm.IBselect.RecNo, fim1,'Gerando Relatório...',false,false,2);
  ...
  funcoes.informacao(dm.IBselect.RecNo, fim1,'Gerando Relatório...',false,true,2);
  addRelatorioForm19(CompletaOuRepete('SUB TOTAL CLIENTE ' + LeftStr(NOMCLI, 35) + ' => ',  formataCurrency(TOTCLI), ' ', 68) + crlf);
  addRelatorioForm19(CompletaOuRepete('', '', '-', 79) + crlf + crlf);
...
var
  sim, prec, h1, h2, h3, ordem : string;
  tam : integer;
begin
  form19.RichEdit1.Clear;
  b := 52;
  ...
  prec := funcoes.dialogo('generico',0,'12'+#8,0,false,'S','Control For Windows','Imprimir Preço de (1 - Venda, 2 - Compra)?:','1') ;
  if (prec = '*') then exit;
  ...
  dm.ProdutoQY.SQL.Add('select cod,unid as UN,emb,codbar,nome as DESCRICAO, '+ prec +' as Preco, quant as ESTOQUE,fabric,fornec,grupo from produto where '+h1+' '+h2+' '+h3+' (cod <> -1) order by ' + ordem);
  dm.ProdutoQY.Open;
...
begin
 B:=52;
 fornec := funcoes.dialogo('generico',0,'1234567890'+#8,0,false,'','Control For Windows','Informe o Código do Fornecedor:','') ;
...
begin
  ...
  if not DirectoryExists(pstaNfe) then
    begin
      MessageDlg('Este Terminal não tem informações das Notas Emitidas', mtError, [mbOK], 1);
      exit;
    end;
       {
...
begin
  ...
  separa := funcoes.dialogo('generico',0,'12',0,false,'S','Control For Windows','Separar Despesas por (1-Históricos 2-Grupo de Caixa) ?','1');
  if separa = '*' then exit;
  ...
  if ICM_VENDA then
    begin
      ...
      funcoes.informacao(0, fi, 'Agurde, Gerando Relatório...', true, false, 5);
      i := 0;

      _COD := dm.IBselect.fieldbyname('cod').AsString;
      while true do //varre os produtos da venda
        begin
          funcoes.informacao(dm.IBselect.RecNo, fi, 'Agurde, Gerando Relatório...', false, false, 5);
  ...
  else funcoes.informacao(0, fi, 'Agurde, Gerando Relatório...', true, false, 5);
  ...
  i := 0;
  funcoes.informacao(i, fi, 'Agurde, Gerando Relatório...', false, false, 5);
 ...
 // addRelatorioForm19(funcoes.CompletaOuRepete('TOTAL DE ENCARGOS                 => ' + formataCurrency(despesas - pag) + '- '+ fim, '', ' ', 80) + #13 + #10 + CRLF);
  ...
  funcoes.informacao(0, fi, 'Agurde, Gerando Relatório...', false, true, 5);
  caixaEnt.Free;
  caixaSai.Free;
  hisNomes.Free;
  dm.IBselect.Close;
...
begin
  ...
  imp_pre := funcoes.dialogo('generico',0,'12',0,false,'S','Control For Windows','Imprimir Preço (1-Varejo 2-Atacado)?','1') ;
...
begin
  ...
  form19.RichEdit1.Clear;
  funcoes.PerguntasRel(dm.produtoqy,'123',false,'','');
  separa:=funcoes.dialogo('generico',0,'1234'+#8,0,false,'S','Control For Windows','Qual a Ordem (1-Alfabética, 2-Fornecedor, 3-Fabricante, 4-Localização) ?','') ;
  //form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((#15+#13))));
  addRelatorioForm19('|' + #15 + '|' + CRLF);
...
begin
  ...
  if fabric<>'*' then sql:=funcoes.dialogo('generico',0,'1234',0,false,'S','Control For Windows','Qual a Ordem (1-Alfabética, 2-Fornecedor, 3-Fabricante, 4-Numérica) ?','1');

  if not funcoes.Contido('*',fabric+sql+SelecaoStr) then ini := funcoes.dialogo('normal',0,'1234567890'+#8,40,false,'','Control For Windows','Qual o Código Inicial?','');
  if not funcoes.Contido('*',fabric+sql+ini) then fim:=funcoes.dialogo('normal',0,'1234567890'+#8,40,false,'','Control For Windows','Qual o Código Final?','');

  if not funcoes.Contido('*',ini+fim+fabric+sql) then SelecaoStr:= funcoes.dialogo('generico',0,'123',0,false,'S','Control For Windows','Qual o Critério de Seleção?(1-Loja+Depósito 2-Loja 3-Depósito)','1');

  lin := funcoes.dialogo('normal',0,'1234567890'+#8,40,false,'','Control For Windows','Imprimir Quantas Linhas por Página ?',IntToStr(linhas));
  if lin = '*' then exit;
                 ...
                 begin
                   if dm.ProdutoQY.fieldbyname('fornec').AsString<>cod then
                     begin
                       ...
                       if (funcoes.BuscaNomeBD(dm.ibquery1,'nome','fornecedor','where cod='+cod))=''  then addRelatorioForm19(strzero(cod, 6)+'-FORNECEDOR NÃO CADASTRADO'#13+#10)
                     ...
                     begin
                       if dm.ProdutoQY.fieldbyname('fabric').AsString<>cod then
                         begin
                           ...
                           if (funcoes.BuscaNomeBD(dm.ibquery1,'nome','fabricante','where cod='+cod))=''  then addRelatorioForm19(strzero(cod, 6)+'-FABRICANTE NÃO CADASTRADO'#13+#10)
...
begin
  ...
  if not funcoes.Contido('*',grupo+estoquezero) then sql:=funcoes.dialogo('generico',0,'14',0,false,'S','Control For Windows','Qual a Ordem (1-Alfabética, 4-Numérica) ?','1');
  //form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((#15+#13))));
  addRelatorioForm19('|' + #15 + '|' + CRLF);
...
begin
  if form22.usuario <> 'ADMIN' then
    begin
      MessageDlg('Este Usuário Não tem Permissão para Acessar essa Rotina.', mtInformation, [mbOK], 1);
      exit;
    end;
  ...
  form40.ListBox1.Items.Add('1=Qual a Sequência de Colunas na Venda(Padrão:0123456789)?'+#13+#13+'0-Código de Sequencia '+#13+'1-Descrição do Produto'+#13+'2-Preço de Venda'+#13+'3-Quantidade'+#13+'4-Unidade'+#13+'5-Código de Barras'+#13+'6-Aplicação'+#13+'7-Referência Original'+#13+'8-Localização'+#13+'9-Depósito');
  form40.ListBox1.Items.Add('2=Confirmar o Valor Total da Nota(S - Reais, N - Porcentagem, X - Ambos)?');
  form40.ListBox1.Items.Add('3=Qual o Padrão Para Cliente Ativo no Cadastro de Clientes (S/N)? ');
  ...
  form40.ListBox1.Items.Add('6=Qual a Taxa Para Correção de Contas a Receber em Atraso (ao dia)? ');
  ...
  form40.ListBox1.Items.Add('10=Qual o Regime de Tributação(1-Simples Nacional 2-Simples C/ Excesso de Sublimite 3-Regime Normal/Lucro Presumido 4-Regime Normal/Lucro Real)? ');
  ...
  form40.ListBox1.Items.Add('15=Usar Informações de Frete na Nota Fiscal Eletrônica? ');
  ...
  form40.ListBox1.Items.Add('18=Qual o Periodo para Calculo do Estoque Mínimo (em dias)?');
  form40.ListBox1.Items.Add('19=Imprime Nota na Venda (S/N)?');
  form40.ListBox1.Items.Add('20=Imprime Parcelamento na Venda (S/N)?');
  form40.ListBox1.Items.Add('21=Exigir Confirmação das notas de Pedido para o Movimento do dia (S/N)?');
  form40.ListBox1.Items.Add('22=Qual a Aliquota Padrão no Cadastro de Produto ?');
  form40.ListBox1.Items.Add('23=Habilitar Funcionamento em Rede?');
  form40.ListBox1.Items.Add('24=Habilitar Saída de Estoque ?');
  form40.ListBox1.Items.Add('25=Comparar Lucro com 1-Custo da Nota, 2-Custo Real, 3-Preço de Venda?');
  form40.ListBox1.Items.Add('26=Usar NCM na Nota Fiscal Eletrônica?');
  form40.ListBox1.Items.Add('27=Imprimir Endereço do Cliente no Cupom da Venda?');
  form40.ListBox1.Items.Add('28=Qual o Desconto para Pagamento a Vista?');  // Padrao  é 20 %
  form40.ListBox1.Items.Add('29=Qual o Desconto para Pagamento em Cartão de Crédito?'); //Padrão é 15%
  form40.ListBox1.Items.Add('30=Usar Cadastro de Produtos em Série na Entrada Simples?');
  ...
  form40.ListBox1.Items.Add('35=Qual o Padrão de Nomes Para o Sistema de Serviços?');
  ...
  form40.ListBox1.Items.Add('38=Qual a Informação contida na Etiqueta da balança (1-Quantidade 2-Preço Total 3-Nenhum) ?');
  ...
  form40.ListBox1.Items.Add('43=Usar Campo Preço de Custo na Consulta de Produtos?');
  ...
  form40.ListBox1.Items.Add('48=Exibir Nome do Cliente no Relatório de Vendas Por Nota (0-Sim 1-Não) ?');
  form40.ListBox1.Items.Add('49=Usar Cód. de Barras na Impressão do Pedido LOCALIZA ?');
  ...
  form40.ListBox1.Items.Add('57=Imprimir Entradas no Caixa no Relatório de Vendas por Nota (S-Ent. e Saidas/N-Nenhum/E-Entradas) ?');
  ...
  form40.ListBox1.Items.Add('60=Usar Cód. de Barras na Tabela de Itens Vendidos (S/N) ?');
  form40.ListBox1.Items.Add('61=Usar Qual Campo na Sub-Consulta de Produtos (S-Refori/N-cod Seq./C-codbar) ?');
  form40.ListBox1.Items.Add('62=Desabilitar Ponteiro Após Consulta Por Código de Barras ?');
  form40.ListBox1.Items.Add('63=Usar Venda de Mercadorias em Peças Sob Medida M2 (S/N) ?');
  ...
  form40.ListBox1.Items.Add('67=Qual a Mensagem Padrão no Pedido de Orçamentos ?');
  ...
  form40.ListBox1.Items.Add('71=Confirmar Preço dos Produtos Sem Gravar como Desconto ?');
  form40.ListBox1.Items.Add('72=Quantidade de dias de vendas para sugestão de compras:');
  form40.ListBox1.Items.Add('73=Usar mensagem individual para cada venda ?');
  form40.ListBox1.Items.Add('74=Imprimir no Pedido de Venda ? (1-Localizacao 2-Ref. Original 3-Cod Barras)');
...
addRelatorioForm19('|' + #15 + '|' + CRLF);
sql:='';
grup := funcoes.dialogo('generico',0,'1234567890'+#8,0,false,'','Control For Windows','Se Deseja Separação por GRUPO, Informe um Cód.:','') ;
...
begin
  ...
  his := funcoes.dialogo('normal',0,'',150,false,'',Application.Title,'Qual o Histórico?','');
  if his = '*' then exit;
  ...
  if ordem = '1' then ordem := ' order by vencimento desc'
    ...
    funcoes.FormataCampos(dm.ibselect,2,'valor',2);
      ...
      on e : exception do
        begin
          ShowMessage('Erro: ' + e.Message + #13 + #10 + 'Verifique se as datas são válidas');
          exit;
        end;
    end;
...
begin
  ...
  nota := funcoes.dialogo('generico',0,'1234567890,.'+#8,100,false,'','Relatório de Entradas','Qual o Número da Nota ?','');
...
begin
  ...
  cli := funcoes.dialogo('generico',0,'1234567890,.'+#8,50,false,'',Application.Title,'Qual o Cód do Cliente?','');
  if cli = '*' then exit;
...
begin
  ...
  dm.IBselect.Close;
  dm.IBselect.SQL.Text := 'select codgru, vencimento, documento, historico, previsao, valor, cod, valor as saldo  from contasreceber where ' +
...
valor[6]:=0;
  ...
  if valor[3]=0 then form28.Chart1.Series[0].Add(0,'Até 8 Dias'+#13+'R$ '+FormatCurr('#,###,###0.00',valor[3]),form28.Chart1.Series[0].ValueColor[0])
  else form28.Chart1.Series[0].Add(int((valor[3]/valor[1])*100),'Até 8 Dias'+#13+'R$ '+FormatCurr('#,###,###0.00',valor[3]),form28.Chart1.Series[0].ValueColor[0]);

  if valor[4]=0 then form28.Chart1.Series[0].Add(0,'Até 15 Dias'+#13+'R$ '+FormatCurr('#,###,###0.00',valor[4]),form28.Chart1.Series[0].ValueColor[0])
  else form28.Chart1.Series[0].Add(int((valor[4]/valor[1])*100),'Até 15 Dias'+#13+'R$ '+FormatCurr('#,###,###0.00',valor[4]),form28.Chart1.Series[0].ValueColor[0]);

  if valor[5]=0 then form28.Chart1.Series[0].Add(0,'Até 30 Dias'+#13+'R$ '+FormatCurr('#,###,###0.00',valor[5]),form28.Chart1.Series[0].ValueColor[0])
  else form28.Chart1.Series[0].Add(int((valor[5]/valor[1])*100),'Até 30 Dias'+#13+'R$ '+FormatCurr('#,###,###0.00',valor[5]),form28.Chart1.Series[0].ValueColor[0]);
...
begin
  ...
  begin
    while his='' do
     begin
       his := funcoes.dialogo('normal',0,'',50,false,'',application.Title,'Qual o Histórico?','');
     end;
  end;
...
begin
  ...
  if imprimirtotaldia = '*' then exit;
    ...
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'Relatório de Contas a Pagar de: '+ini+' a '+fim,'HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'#13+#10,'-',81)))));
...
begin
  ...
  if not funcoes.Contido('*',grupo+ini+fim) then his := funcoes.dialogo('normal',0,'',50,false,'',Application.Title,'Qual o Histórico?','');
  if not funcoes.Contido('*',grupo+ini+fim+his) then imprimirtotaldia := funcoes.dialogo('generico',0,'SN',20,false,'S',Application.Title,'Imprimir Endereço dos Clientes?','N');
  if not funcoes.Contido('*',grupo+ini+fim+his+imprimirtotaldia) then
   begin
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+funcoes.LerValorPGerais('empresa',form22.Pgerais),'DATA: '+FormatDateTime('dd/mm/yy',now)+'|'+#13+#10,' ',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'Relatório de Contas a Receber de: '+ini+' a '+fim,'HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'#13+#10,'-',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar(('|Caixa| Vencto | Nr.Doc |   Historico                             |   Valor   |'+#13+#10))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
     if grupo<>'' then g1 := '(codgru='+grupo+') and';
...
begin
  ...
  if not funcoes.Contido('*',grupo+ini+fim+his) then
   begin
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+funcoes.LerValorPGerais('empresa',form22.Pgerais),'DATA: '+FormatDateTime('dd/mm/yy',now)+'|'+#13+#10,' ',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'Relatório de Contas a Receber Por Localidade: '+his,'HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'#13+#10,'-',81)))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar(('|Caixa| Vencto | Nr.Doc |   Historico                             |   Valor   |'+#13+#10))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
...
begin
  ...
  if not funcoes.Contido('*',grupo) then ini := funcoes.dialogo('data',0,'',2,true,'',Application.Title,'Qual a Data Inicial?',formatadataddmmyy(StartOfTheMonth(now)));
  if not funcoes.Contido('*',grupo+ini) then fim := funcoes.dialogo('data',0,'',2,true,'',Application.Title,'Qual a Data Final?',formatadataddmmyy(endOfTheMonth(now)));
  if not funcoes.Contido('*',grupo+ini+fim) then his := funcoes.dialogo('normal',0,'',50,false,'',Application.Title,'Qual a Observação?','');
   if not funcoes.Contido('*',grupo+ini+fim+his) then
   begin
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+funcoes.LerValorPGerais('empresa',form22.Pgerais),'DATA: '+FormatDateTime('dd/mm/yy',now)+'|'+#13+#10,' ',81)))));
...
begin
  ...
  if not funcoes.Contido('*',grupo+ini+fim) then  his := funcoes.dialogo('normal',0,'',20,false,'',Application.Title,'Qual o Vendedor?','');
   ...
   begin
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+funcoes.LerValorPGerais('empresa',form22.Pgerais),'DATA: '+FormatDateTime('dd/mm/yy',now)+'|'+#13+#10,' ',81)))));
    if his<>'' then form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'Relatório de Contas a Receber Por Vendedor: '+funcoes.BuscaNomeBD(dm.ibquery2,'nome','vendedor',' where cod='+his),'HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',81)))))
     else form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'Relatório de Contas a Receber Por Vendedor: Não Informado','HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',81)))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'#13+#10,'-',81)))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar(('|Caixa| Vencto | Nr.Doc |   Historico                             |   Valor   |'+#13+#10))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
...
begin
  ...
  if not funcoes.Contido('*',grupo+ini+fim+saldo) then  resumo := funcoes.dialogo('generico',0,'SN',20,false,'S',Application.Title,'Imprimir Resumo por Histórico?','N');
   ...
   begin
    ...
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('| Razão Social:'+nome+'                Pagina:'+IntToStr(pag),'DATA: '+FormatDateTime('dd/mm/yy',form22.datamov)+'|',' ',116)+#13+#10))));
...
begin
  ...
  his := funcoes.dialogo('generico',0,'1234567890'+#8,50,false,'',Application.Title,'Informe o Cód. do Histórico?','');
  if his = '*' then exit;

  if his = '' then begin
    //his := funcoes.localizar('Localizar Histórico','hiscaixa','cod','cod','cod','grupo','grupo',false,false,false,'',0,sender);
    His := funcoes.localizar('Localizar Histórico','hiscaixa','cod,grupo','cod,grupo','','grupo','grupo',true,false,false,'',0,nil);
...
if (usu<>'*') and (usu<>'') then
 ...
 //0-estoque 1-financas 2-posicao 3-relatorio 4-cadastros 5-utilitarios 6-serviços
  if (dm.IBQuery2.IsEmpty) then ShowMessage('Usuário Não Encontrado!')
   else
    begin
...
var
  ...
  i : integer;
begin
  sim := funcoes.dialogo('generico',0,'SN',30,false,'S',Application.Title,'Imprimir Clientes que Estão em Dia?','N');
  if sim = '*' then exit;
...
begin
  ...
  if not funcoes.Contido('*',grupo+ini+fim+formpagto) then
   begin
    ...
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'Relatório de Contas a Receber Por Forma de Pagamento:'+codigos,'HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',107)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'#13+#10,'-',107)))));
...
var
  ini,fim,ende,bairro,est,cid,empresa,telres,telcom,cli:string;
  linhas,i : integer;
  total : currency;
begin
  ini := funcoes.dialogo('data',0,'',2,true,'',Application.Title,'Qual a Data Inicial?',formatadataddmmyy(StartOfTheMonth(now)));
...
begin
  ...
  form19.RichEdit1.Clear;
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+funcoes.LerValorPGerais('empresa',form22.Pgerais),'DATA: '+FormatDateTime('dd/mm/yy',now)+'|'+#13+#10,' ',81)))));
    form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'RELATÓRIO MÉDIA DE DIAS DE ATRASO:: CLIENTE => '+cli,'HORA: '+FormatDateTime('tt',now)+'|'+#13+#10,' ',81)))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'#13+#10,'-',81)))));
    ...
    while not dm.IBselect.Eof do
     ...
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('|'+'ATRASO MÉDIO => '+IntToStr(trunc(totalD/cont))+' DIAS'+'   TOTAL PAGO => '+FormatCurr('#,###,###0.00',totalgeral),'|',' ',79)+#13+#10))));
     form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('+','+'+#13+#10,'-',81)))));
     form19.RichEdit1.SelStart := 0;
     form19.ShowModal;
end;
...
begin
  ...
  nota := funcoes.dialogo('generico',0,'1234567890'+#8,100,false,'',Application.Title,'Informe o Número Da Nota:',nota);
  if nota = '*' then exit;
...
if funcoes.lista1 = '0' then
 begin
   usu := funcoes.dialogo('normal',0,'',0,false,'',Application.Title,'Qual o Usuário?',form22.usuario);
   if usu = '*' then exit;
   dm.IBselect.Close;
   dm.IBselect.SQL.Clear;
   ...
   form36.ListBox1.Items.Add('0-Qual o Desconto Máximo Permitido (de 0 a 99%)?');
   form36.ListBox1.Items.Add('1-Qual a Nota de Venda(T-Ticket,M-Média,G-Grande,L-Localiza,A-Atacado,R-Resumo,D-Dav, F-Ticket Gráfico)?');
   form36.ListBox1.Items.Add('2-Confirmação para o preço de produto na venda (S-Reais P-Porcent, X-Ambos, N-Nenhum) ?');
   form36.ListBox1.Items.Add('3-Imprimir Mensagem na Nota de Pedido(S-Sim  N-Não  X-Apenas Orçamentos)?');
   form36.ListBox1.Items.Add('4-Imprime Endereço do Cliente no Ticket?');
   form36.ListBox1.Items.Add('5-Solicitar Código do Cliente nas Vendas a Vista?');
   form36.ListBox1.Items.Add('6-Considerar Limite de Crédito do Cliente nas Vendas a Prazo?');
   ...
   form36.ListBox1.Items.Add('9-Permitir a Impressão de Orçamento (S-Sim/N-Nao/X-Perguntar)?');
 ...
 begin
   ...
   form39.ListBox1.Clear;
   form39.conf := 2;
   form39.ListBox1.Items.Add('0=Qual a Nota de Venda(T-Ticket,M-Média,G-Grande,L-Localiza,A-Atacado,R-Resumo,D-Dav,F-Ticket Gráfico)?');
   ...
   form39.ListBox1.Items.Add('3=Qual o Tamanho da Fonte da Impressão (Ticket ou DAV)? ');
   ...
   form39.ListBox1.Items.Add('8=Qual o tamanho do campo DESCRIÇÃO na impressao do pedido de compra ?');
...
begin
 ...
 nota := funcoes.dialogo('generico',0,'1234567890'+#8,90,false,'',Application.Title,'Informe o Número Da Nota:',nota);
 if ((nota = '*') or(nota = '')) then exit;
 ...
 if not(dm.IBQuery2.IsEmpty) then funcoes.GeraNota(nota,form22.Pgerais.Values['nota'], 'N', 1)
  else ShowMessage('Nota Não Encontrada!');
 dm.IBQuery2.Close;
end;
...
begin
  ...
  imp_ent := funcoes.buscaParamGeral(57, 'S');    
    ...
    if form22.Pgerais.Values['nota'] = 'T' then
     ...
     funcoes.informacao(i, fi, 'Agurde, Gerando Relatório...', true, false, 5);
     ...
     while not dm.IBselect.Eof do
       begin
         ...
         if form22.Pgerais.Values['nota'] = 'T' then
           begin
             caixa.Add(FormatDateTime('dd/mm/yy',dm.IBselect.fieldbyname('data').AsDateTime)+' '+funcoes.CompletaOuRepete('',dm.IBselect.fieldbyname('historico').AsString,' ',35)+funcoes.CompletaOuRepete('',FormatCurr('#,###,###0.00',VLR_ICM) + IfThen(b = 1, '-', '+'),' ',16));
           end
         else caixa.Add(FormatDateTime('dd/mm/yy',dm.IBselect.fieldbyname('data').AsDateTime)+' '+ strzero(dm.IBselect.fieldbyname('documento').AsString, 6) +' ' +funcoes.CompletaOuRepete(dm.IBselect.fieldbyname('historico').AsString,'',' ',35)+funcoes.CompletaOuRepete('',FormatCurr('#,###,###0.00', VLR_ICM) + IfThen(b = 1, '-', '+'),' ',16));
     ...
     funcoes.informacao(i, fi, 'Agurde, Gerando Relatório...', false, true, 5);
     caixa.Free;
     totais.Free;
     dm.IBselect.Close;
     form19.RichEdit1.SelStart := 0;
...
i : integer;
begin
  cliente := funcoes.dialogo('generico',100,'1234567890'+#8,100,false,'',Application.Title,'Qual o Código do cliente?','');
...
var
  ...
  i : integer;
begin
  cliente := funcoes.dialogo('generico',30,'1234567890'+#8,30,false,'',Application.Title,'Qual o Código do cliente?','');
  if (cliente = '*') then exit;
  {if cliente = '' then cliente := funcoes.localizar('Localizar Cliente','cliente','cod,nome,telres,telcom,cnpj,bairro','cod','','nome','nome',false,false,false,'',0,nil);
  if (cliente = '') or (cliente = '*') then exit;
  ...
  while not dm.IBselect.Eof do
    begin
      if clix <> dm.IBselect.FieldByName('cliente').AsString then begin
        ...
        form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((funcoes.CompletaOuRepete('CLIENTE: '+dm.IBselect.FieldByName('cliente').AsString+'-'+funcoes.BuscaNomeBD(dm.ibquery1,'nome','cliente','where cod='+dm.IBselect.FieldByName('cliente').AsString),'',' ',80)+#13+#10))));
        form19.RichEdit1.Perform(EM_REPLACESEL, 1, Longint(PChar((CRLF))));
...
begin
  ...
  minim := funcoes.dialogo('generico',0,'1234567890'+#8+#27+#13,50,true,'',Application.Title,'Qual a Quantidade Mínima Vendida?','1');
  if minim = '*' then exit;
...
begin
  fornec := funcoes.dialogo('generico',3,'1234567890'+#8,60,true,'','Restaurar Cadastro','Qual o Código ?','');
  if fornec = '*' then exit;
...
begin
  ...
  try
    dm.IBQuery1.ExecSQL;
    dm.IBQuery1.Transaction.Commit;
    ShowMessage('Algumas Configurações Serão Acionadas Após o Reinício do Sistema.');
  except
    dm.IBQuery1.Transaction.Rollback;
    ShowMessage('Ocorreu Um Erro. Tente Novamente!');
...
begin
  ...
  his := funcoes.dialogo('not',0,'',50,true,'',Application.Title,'Confirme o Histórico:','');
  if his = '*' then exit;
...
begin
   b := 60;
   cond := '';
  completo := funcoes.dialogo('generico',0,'SN',0,false,'S',Application.Title,'Imprimir Relatório Completo com Endereços?','S');
  ...
  addRelatorioForm19(funcoes.CompletaOuRepete('RELATÓRIO DE CLIENTES ','HORA: '+FormatDateTime('tt',now),' ',80)+#13+#10);
  addRelatorioForm19(funcoes.CompletaOuRepete('','','-',80)+#13+#10);
...
var
  temp : string;
begin
  temp := funcoes.dialogo('normal',0,'SN'+#8,0,false,'S','Control For Windows','Atenção, altere esta tabela somente com orientação de uma pessoa'+#13'autorizada e que conheça a Legislação do ICMS do seu estado. Qualquer'+#13+'mudaça nos dados desta tabela alterará o calculo do ICMS na emissão'+#13+'da Nota Fiscal. Estes dados não alteram a emissão de Cupom Fiscal,'+#13+'pois o equipamento Fiscal já vem programado pelo revendedor'+#13+'autorizado. Deseja Continuar (S/N) ?','N');
  ...
  if nota = '' then
    begin
      ...
      {if dm.IBQuery2.IsEmpty then
        begin
          dm.IBQuery2.Close;
          ShowMessage('Não existe Vendas Pendentes na Data Atual');
          exit;
        end;}
...
//      valor := dm.IBselect.fieldbyname('total').AsCurrency;
    ...
    begin
     ...
     totVenda := valor;
        ...
        begin
          ...
          if ((pag = '2')) then
            begin
              if formpagto <> '2' then
                begin
                  ShowMessage('Forma de Pagamento a A PRAZO não pode ser trocada no recebimento. '+ #13 +' Escolha outra forma de Pagamento');
                  err1 := err1 + 1;
                  exit;
                end;
            end;
          ...
          if err1 = 2 then
            begin
              MessageDlg('Forma de Pagamento Inválida, Favor Escolha Outra Forma de Pagamento!', mtInformation, [mbOK],1);
              exit;
            end;
      ...
      //MessageText('O valor do Troco é => ' + FormatCurr('#,###,###0.00', StrToCurr(total) - valor),HexToTColor('FFD700'),true,false, HexToTColor('B22222'));
      ...
      try
        ...
        if MessageBox(handle, 'Deseja Emitir Cupom para Esta Venda', 'Cupom Eletrônico', MB_YESNO + MB_DEFBUTTON2) = idyes then
          begin
...
begin
   avis := funcoes.dialogo('numero',0,'',2,false,'S',Application.Title,'Qual o Percentual de Comissão À VISTA (%)?','0,00');
   if avis = '*' then exit;

   atra := funcoes.dialogo('numero',0,'',2,false,'S',Application.Title,'Qual o Percentual de Comissão À PRAZO (%)?', avis);
   if atra = '*' then exit;

   dif := funcoes.dialogo('numero',0,'',2,true,'S',Application.Title,'Qual o fator de Redução A PRAZO para produtos diferenciados?','1,00');
   if dif = '*' then exit;
   ...
   { dm.IBselect.SQL.Clear;
    ...
    funcoes.informacao(0, fim, 'Aguarde, Calculando Comissões...', true, false, 5);
...
begin
   avis := funcoes.dialogo('numero',0,'',2,true,'S',Application.Title,'Qual o Percentual de Comissão À VISTA (%)?','0,00');
   if avis = '*' then exit;
   ...
   fim1 := dm.IBselect.RecordCount;
   ini1 := 0;
   funcoes.informacao(ini, fim, 'Aguarde, calculando Comissões...', true, false, 5);

   while not dm.IBselect.Eof do
     begin
       ini1 := ini1 + 1;
       funcoes.informacao(ini1, fim1, 'Aguarde, calculando Comissões...', false, false, 5);
       dm.IBQuery1.Close;
       dm.IBQuery1.SQL.Clear;
   ...
   funcoes.informacao(ini, fim, 'Aguarde, calculando Comissões...', false, true, 5);
   fim := lista.Count - 1;
   total := 0;
   for i := 0 to fim do
     begin
...
begin
   ...
   atra := funcoes.dialogo('numero',0,'',2,true,'S',Application.Title,'Qual o Percentual de Comissão para o Vendedor (%)?','0,00');
   if atra = '*' then exit;

   vende := funcoes.dialogo('generico',0,'0987654321' + #8 + #27,50, false,'',Application.Title,'Qual o Número do Vendedor?','');
   {if vende = '' then
     begin
   ...
   dif := funcoes.dialogo('generico',0,'SN',50,true,'S',Application.Title,'Pagar Comissão Sobre Vendas não Recebidas?','N');
   if dif = '*' then exit;
...
begin
  ...
  ordem := funcoes.dialogo('generico',0,'123'+#8,0,false,'S','Control For Windows','Qual a Ordem  (1-Alfabética 2-Diferença 3-Sugestão)?:','1') ;
  if ordem = '*' then exit;
...
begin
  ...
  if not FileExists(caminhoEXE_com_barra_no_final + 'errolog.txt') then
     begin
       ShowMessage(caminhoEXE_com_barra_no_final + 'errolog.txt' + ' Arquivo não Econtrado!');
       exit;
     end;
  ...
  texto := texto + 'Endereço: ' + dm.IBselect.fieldbyname('ende').AsString + ' - ' + dm.IBselect.fieldbyname('bairro').AsString + #13 + #10;
  ...
  MAILMESSAGE.Body.SetText(pchar(texto));
  ...
  MAIL.Send(MAILMESSAGE);
     ...
     pergunta1.Close;
     pergunta1.Free;
     ShowMessage('Não Foi Possível Enviar o Email. Verifique se a Conexão com a Internet está Funcionando Corretamente');
...
begin
  ...
  while not dm.IBselect.Eof do begin
    funcoes.informacao(dm.IBselect.RecNo, ultimo, 'Calculando Estoque Mínimo... ',false,false,2);
  ...
  funcoes.informacao(0, ultimo,'Calculando Estoque Mínimo... ',true,false,2);
  while not dm.IBQuery3.Eof do
    begin
      funcoes.informacao(dm.IBQuery3.RecNo, ultimo, 'Calculando Estoque Mínimo... ',false,false,2);
  ...
  dm.IBQuery1.Close;
  dm.IBQuery3.Close;
  ShowMessage('Calculo de Estoque Mínimo Concluído com Sucesso. (' + inttostr(alterados) + ' produtos alterados)');
{var
...
begin
  ...
  if (ssCtrl in Shift) and (chr(Key) in ['O', 'o']) then
    begin
      cod := funcoes.dialogo('generico',50,'1234567890'+#8,50,false,'','Control For Windows','Qual o Código do Menu ?','') ;
      cliqueMenu(COD);
    end;
...
begin
   ...
   while true do
     begin
       saidaJustificativa   := funcoes.dialogo('normal', 0, '',100, false,'','Justificativa de Saída','Qual a Justificativa?', saidaJustificativa); //funcoes.MensagemTextoInput('Justificativa De Saída', '');
...
begin
  ...
  TOTGERAL := 0;
  dataInv := '';
  dataInv := funcoes.dialogo('data',0,'',2,true,'',Application.Title,'Confirme a data do inventário: ',formatadataddmmyy(EndOfTheMonth(StrToDateTime('01/12/' + FormatDateTime('yyyy', now)))));
  if dataInv = '*' then exit;

  base := funcoes.dialogo('not', 0, 'SNA', 20, true, 'S', Application.Title, 'Calcular com base no Preço da Venda (S-Sim, N-Não, A-Atacado) ?', 'N');
  if base = '*' then exit;

  estoqueZero := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Imprimir Itens Com Estoque Zero (S-Sim, N-Não) ?', 'N');
  if estoqueZero = '*' then exit;

  fornec := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, false, '', Application.Title, 'Se Deseja Separação Por FORNECEDOR, Confirme com o Código:', '');
  if fornec = '*' then exit;

  fabric := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, false, '', Application.Title, 'Se Deseja Separação Por FABRICANTE, Confirme com o Código:', '');
  if fabric = '*' then exit;

  aliquota := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, false, '', Application.Title, 'Se Deseja Separação Por ALIQUOTA, Confirme com o Código:', '');
  if aliquota = '*' then exit;

  codigo := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, false, '', Application.Title, 'Se Deseja Separação Por CODIGO, Qual o Código Inicial?', '');
  if codigo = '*' then exit;

  grupo := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, false, '', Application.Title, 'Se Deseja Separação Por GRUPO, Confirme com o Código:', '');
  if grupo = '*' then exit;

  ordem := funcoes.dialogo('not', 0, '123', 50, true, 'S', Application.Title, 'Qual a Ordem do Relatório (1-Alfabética, 2-Fornecedor, 3-Fabricante) ?', '1');
  if ordem = '*' then exit;

  selecao := funcoes.dialogo('normal', 0, '123', 50, true, 'S', Application.Title, 'Qual o Critério de Seleção (1-Loja + Depósito, 2-Somente Loja, 3-Somente Depósito) ?', '1');
...
{  if selecao = '' then
  ...
  arredond := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Arrendondar Quantidades para Inteiro (S-Sim, N-Não)?', 'S');
  if arredond = '*' then exit;

  impri := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Enviar para Impressora (S-Sim, N-Não) ?', 'N');
  if impri = '*' then exit;

 // if impri = 'S' then
 //   begin
      colIni := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, true, '', Application.Title, 'Confirme a Coluna Inicial de Impressão:', '15');
      if colIni = '*' then exit;
//    end;

  seqIni := funcoes.dialogo('generico', 0, '1234567890' + #8, 50, true, '', Application.Title, 'Numeração de Paginas Inicia em:', '1');
  if seqIni = '*' then exit;
...
begin
  ...
  ord := funcoes.dialogo('generico', 0, '12', 50, true, 'S', Application.Title, 'Qual a Ordem (1-Descrição, 2-Código)', '1');
  if ord = '*' then exit;
  b := 64;
...
var
  ...
  ValorFecha : integer;
begin
  sim := funcoes.dialogo('generico', 20, 'SN'+#8, 20, true,'S','Control For Windows','Confirma Fechamento de Vendas?SIM ou NÃO (S/N):','') ;
  if (sim = '*') or (sim = 'N') then exit;

  vend := funcoes.dialogo('generico',0,'1234567890'+#8,0,false,'','Control For Windows','Informe o Código do Vendedor:','') ;
  if vend = '*' then exit;
  ...
  imp := funcoes.dialogo('generico',0,'SN'+#8,0,true,'S','Control For Windows','Enviar para Impressora?SIM ou NÃO (S/N):','') ;
  if imp = '*' then exit;
...
begin
  WWMessage('Atenção! Todos os Itens do Estoque Terão suas Quantidades Zeradas!',mtInformation,[mbOK],HexToTColor('FFD700'),true,false,HexToTColor('B22222'));
...
begin
 ...
 if not dm.IBselect.IsEmpty then
   begin
     WWMessage('Esta nota não pode ser inserida pois já existe uma nota com a numeração ' + nota + '.',mtInformation,[mbOK],HexToTColor('FFD700'),true,false,HexToTColor('B22222'));
     dm.IBselect.Close;
 ...
 sim := funcoes.dialogo('generico',0,'SN'+#8,0,true,'S','Control For Windows','Confirma Recebimento da Nota '+ nota +' no Valor de R$ '+ FormatCurr('#,###,###0.00', total) +' ?SIM ou NÃO (S/N):','') ;
    ...
    begin
      ...
      if dm.IBselect.IsEmpty then
        begin
          ...
          if form33.ClientDataSet1.fieldbyname('codigo').AsInteger > StrToInt(genProd) then // Se o código deste produto for maior que
...
var
  ...
  ini, fim : integer;
begin
  data := funcoes.dialogo('generico',0,'1234567890'+#8,0,false,'','Control For Windows','Qual o Mês de Aniversário?', '1') ;
  if data = '*' then exit;
  ...
  ShowMessage('O arquivo foi criado em:' + #13 + pastaMala + 'ANIVERSARIANTES.TXT' + #13 + 'Está disponível um atalho na area de trabalho.');
end;
...
begin
  ...
  while not dm.IBselect.Eof do
   ...
   CloseFile(F);
   cds.Free;
   ShowMessage('O arquivo foi criado em:' + #13 + pastaMala + 'ATRASO.TXT' + #13 + 'Está disponível um atalho na area de trabalho.');
end;
...
begin
  if not funcoes.senhaDodia then exit;
  WWMessage('Atenção! Esta Rotina Fará Reajuste Geral de Preços de Todo o Estoque',mtInformation,[mbOK],HexToTColor('FFD700'),true,false,HexToTColor('B22222'));
  ...
  sim := funcoes.dialogo('generico',0,'SN'+#8,0,false,'S','Control For Windows','Tem Certeza que deseja reajustar os preços em '+ formataCurrency(perc) +'% ?S/N','N') ;
  if ((sim = '*') or (sim = 'N')) then exit;
  ...
  while not dm.IBselect.Eof do
    begin
      ini := ini + 1;
      funcoes.informacao(ini, fim, 'Aguarde, Reajustando Preços...', false, false, 2);
...
begin
  if not VerificaAcesso_Se_Nao_tiver_Nenhum_bloqueio_true_senao_false then
    begin
      WWMessage('Rotina permitida apenas para usuários sem bloqueios',mtInformation,[mbOK],HexToTColor('FFD700'),true,false,HexToTColor('B22222'));
      exit;
    end;  
  ...
  sim := funcoes.dialogo('generico',0,'SN'+#8,0,false,'S','Control For Windows','Confirma reajuste de preço de atacado com desconto de  '+ formataCurrency(perc) +'% ?S/N','N') ;
  if ((sim = '*') or (sim = 'N')) then exit;
  ...
  while not dm.IBselect.Eof do
    begin
      funcoes.informacao(dm.IBselect.RecNo, fim, 'Reajustando Preço de Atacado...', false, false, 2);
...
begin
  ...
  piini := funcoes.dialogo('numero',0,'',2,true,'S',Application.Title,'Qual o Preço Inicial  ?','0,00');
  if piini = '*' then exit;

  pifim := funcoes.dialogo('numero',0,'',2,true,'S',Application.Title,'Qual o Preço Final ?','0,00');
  if pifim = '*' then exit;
  ...
  sim := funcoes.dialogo('generico',0,'SN'+#8,0,false,'S','Control For Windows','Confirma reajuste de '+ formataCurrency(perc) +'% para Preços Entre '+ formataCurrency(pini) +' e '+ formataCurrency(pfim) +' ?S/N','N') ;
  if ((sim = '*') or (sim = 'N')) then exit;
  ...
  funcoes.informacao(1,2,'Reajustando Preços de Atacado...',false, true, 2);
...
begin
  ...
  grupo := funcoes.dialogo('generico',30,'1234567890' + #8,70,false,'',Application.Title,'Qual o Código do Grupo ?', '');
  if prodzero = '*' then exit;
  ...
  while not dm.IBselect.Eof do
    begin
      funcoes.informacao(i, fi, 'Aguade, Gerando Relatório...', false, false, 2);
      i := i + 1;
...
begin
  ...
  porc := funcoes.dialogo('numero',0,'',2,true,'S',Application.Title,'Qual o Percentual de Comissão (%)?','0,00');
  if porc = '*' then exit;
...
begin
  ...
  CODINI := funcoes.dialogo('generico',90,'1234567890'+#8, 90, true,'','Control For Windows','Qual o Cód. Inicial?','') ;
  if CODINI = '*' then exit;

  CODFIM := funcoes.dialogo('generico', 90,'1234567890'+#8, 90, true,'','Control For Windows','Qual o Cód. Final?','') ;
  if CODFIM = '*' then exit;
...
var
  ...
  ini, fim : integer;
begin
  aliq := funcoes.dialogo('generico',50,'1234567890'+#8,50,false,'','Control For Windows','Informe o Código da Aliquota:','') ;
  if aliq = '*' then exit;

  if aliq = '' then aliq := funcoes.localizar('Localizar Aliquota', 'aliq', 'cod, aliq', 'cod', '', '', 'cod', false, false, false, '', 500, nil);
  if aliq = '' then
    begin
      if MessageBox(Handle, 'Deseja gerar este relatório de todos o produtos', pchar(Application.Title), MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = idno then exit;
    end;
...
begin
  ...
  cons := funcoes.dialogo('generico',0,'EC',0,false,'S','Control For Windows','Considerar: E-Data de Emissão C-Data de Chegada?','C') ;
  if cons = '*' then exit;
...
begin
  ...
  tipoPreco := funcoes.dialogo('generico',0,'SN',30,true,'S','Relatório de Entradas','Imprimir Preço de Venda ?','N');
...
begin
  ...
  grupo := funcoes.dialogo('generico',0,'1234567890'+#8,50, true,'',Application.Title,'Qual o Cód. da Ordem de Serviço?', grupo);
  if grupo = '*' then exit;
...
begin
  if not VerificaAcesso_Se_Nao_tiver_Nenhum_bloqueio_true_senao_false then
   begin
     WWMessage('Somente um Usuário Autorizado Pode Cancelar Esta Conta.',mtError,[mbok],HexToTColor('FFD700'),true,false,HexToTColor('B22222'));
     exit;
   end;

  grupo := funcoes.dialogo('generico',0,'1234567890'+#8,50, true,'',Application.Title,'Qual o Cód. da Ordem de Serviço?', '');
  if grupo = '*' then exit;
...
begin
  ...
  grupo := funcoes.dialogo('generico',0,'123',0,true,'S','Control For Windows','Qual o Critério de Seleção (1-Pendentes 2-Encerradas 3-Todas)?','1') ;
  if grupo = '*' then exit;
  ordem := '';
...
begin
  try
    funcoes.Mensagem(Application.Title ,'Aguarde, Verificando Estado do Serviço...',15,'Courier New',false,0,clred);
    Application.ProcessMessages;
    Application.ProcessMessages;
    Verifica_Status_NFe();
  finally
...
var
  serie1 : String;
begin
  nota := funcoes.dialogo('not',0,'1234567890'+#8+#32,50,true,'',Application.Title,'Qual a Número da Nota Fiscal ?','');
  if nota = '*' then exit;
...
var
  ...
  i : integer;
begin
  produto := funcoes.dialogo('generico',100,'1234567890'+#8,100,false,'',Application.Title,'Qual o Código do Produto ?','');
...
begin
  ...
  prodzero := funcoes.dialogo('generico',30,'SN',20,true,'S',Application.Title,'Imprimir Produtos (S-Vendidos N-Não Vendidos A-Ambos) ?', 'A');
  if prodzero = '*' then exit;

  grupo := funcoes.dialogo('generico',30,'1234567890' + #8,20,false,'S',Application.Title,'Qual o Código do Grupo ?', '');
  if prodzero = '*' then exit;
...
begin
  ...
  ShowMessage('Total de Cupons Não Enviados: ' + IntToStr(dm.IBselect.RecordCount));
  dm.IBselect.Close;
end;
...
begin
  vend := '';
  H1 := '';
  vend := funcoes.dialogo('generico',30,'1234567890'+#8,30, FALSE,'',Application.Title,'Qual o Código do Vendedor?','');
  if (vend = '*') then exit;
  ...
  sim := funcoes.dialogo('generico',0,'SN',20,false,'S',Application.Title,'Imprimir Preço das Mercadorias?','N');
  if sim = '*' then exit;
  ...
  while not dm.IBselect.Eof do
    begin
      funcoes.informacao(dm.IBselect.RecNo, fim1, 'Aguarde, Gerando Relatório...', false, false, 5);
      ...
      dm.IBselect.Close;
      desc := 0;
      funcoes.informacao(dm.IBselect.RecNo, fim1, 'Aguarde, Gerando Relatório...', false, true, 5);
      form19.ShowModal;
  end;
...
begin
  ...
  sim := funcoes.dialogo('generico',0,'123',30,false,'S',Application.Title,'Imprimir Contas (1-Todas, 2-Pagas, 3-Não Pagas) ?','1');
  if sim = '*' then exit;
...
begin
  numIni := funcoes.dialogo('not',0,'1234567890'+#8+#32,50,true,'',Application.Title,'Qual a Numeração Inicial ?','');
  if numIni = '*' then exit;

  numFim := funcoes.dialogo('not',0,'1234567890'+#8+#32,50,true,'',Application.Title,'Qual a Numeração Final ?','');
  if numFim = '*' then exit;
...
begin
  ...
  _ser := funcoes.dialogo('not',0,'1234567890'+#8+#32,50,true,'',Application.Title,'Qual a Série ?', getSerieNFCe);
  if _ser = '*' then exit;
  ...
  while length(trim(Justificativa)) < 15 do
   ...
   try
     inutilizacaoNFCE(StrToInt(numIni), StrToInt(numFim), 65, Justificativa, StrToInt(_ser));
   except
     on e:exception do
       begin
         ShowMessage(e.Message);
                  ...
                  if cont = cod1 then
                    begin
                      if MainMenu1.Items[i1].Items[i2].Visible = false then
                        begin
                          MessageDlg('Este Usuário Não tem Permissão para Usar Esta Rotina!', mtError, [mbOK], 1);
                          exit;
                        end;
...
begin
  dm.IBselect.Close;
  dm.IBselect.SQL.Text := 'select * from nfce where (data < ''01.01.1900'') or (data is null) ';
  dm.IBselect.Open;
  ...
  while not dm.IBselect.Eof do begin
    cont := cont + 1;
    if dm.IBselect.FieldByName('chave').AsString <> '' then begin
      dia  := 1;
      ano  := YearOf(now);
      mes  := StrToInt(copy(dm.IBselect.FieldByName('chave').AsString, 5, 2));
      data := EncodeDate(ano, mes, dia);

      dm.IBQuery1.Close;
      dm.IBQuery1.SQL.Text := 'update nfce set data = :data where (chave = :chave)';
      dm.IBQuery1.ParamByName('data').AsDate   := data;
...
begin
  ...
  if dm.IBselect.IsEmpty then
    begin
      dm.IBselect.Close;
      MessageDlg('Não Existe Venda Cancelada Nesse Período!', mtInformation, [mbOK], 1);
      exit;
    end;
...
begin
  ...
  'where i.validade >= ''01.01.2014'' '+h1+' order by i.cod, i.validade desc'; //jss-acrescentei ordem e campo validade
    ...
    begin
      ...
      if idx = -1 then
        begin
          ...
          produtos[idx].quant  := dm.IBselect.fieldbyname('estoque').AsCurrency; //estoque atual(loja + depÃ³sito)
