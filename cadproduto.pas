...
type
  TForm9 = class(TForm)
    ...
    procedure codbarKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
...
begin
  ...
  begin
    codbar1.Add(dm.IBselect.fieldbyname('cod').AsString + '=' +
...
begin
  ...
  dm.IBQuery3.SQL.Clear;
  dm.IBQuery3.SQL.Add
    ('select c.cod, (select nome from produto p where (p.cod = c.cod)) , c.codbar from codbarras c where (cod = :cod) ');
...
function TForm9.CALCLUCRO1(var _LUCRO, _PVENDA, _DEBICM, _BASEDEB, _CREDICM,
  _PCOMPRA, _FRETE, _ENCARGO, _BASECRED, AGREG: JsEditNumero;
  icmsSubsti, descComp: currency; Key: Char): currency;
var
...
begin
  ...
  IF (_BASEDEB.getValor + _BASECRED.getValor + _FRETE.getValor +
    _ENCARGO.getValor + AGREG.getValor + icmsSubsti + descComp) = 0 then
  begin
...
begin
  ...
  // descComp := 0;
  if descComp <> 0 then
    descComp := precoCompra * (descComp / 100);

  // icmsSubsti := 0;
  if icmsSubsti <> 0 then
  ...
  IF _BASEDEB.getValor <> 0 then
  begin
    if AGREG.getValor = 0 then
    begin
      _DEBICM.setValor(ARREDONDA(((_PVENDA.getValor * _BASEDEB.getValor) /
        100), 2));
    end
    else
    begin
      _DEBICM.setValor
        (ARREDONDA(((precoCompra + (precoCompra * AGREG.getValor / 100)) *
        _BASEDEB.getValor / 100), 2));
    end;
  ...
  TMP := _PVENDA.getValor - (precoCompra - _CREDICM.getValor - descComp) -
    _DEBICM.getValor - _FRE - _ENC - icmsSubsti;
...
begin
  try
    ...
    dm.IBselect.SQL.Clear;
    dm.IBselect.SQL.Add
      ('select cod, nome, unid, codbar, refori, aliquota, p_compra, p_venda, quant from produto where cod = :cod');
...
Function TForm9.RelSomatoria_T_(var lista1: Tlist; dini, dfim: String;
  geral: currency): AnsiString;
var
  t1: currency;
...
begin
  ...
  begin
    if ((Length(StrNum(classif.Text)) <> 8) or
      (procuraNCM_Na_Tabela(StrNum(classif.Text)) = false)) then
    begin
      MessageDlg('NCM Inválido, Apague esse código ou preencha corretamente!',
  ...
  if Contido('-' + unid.Text + '-', '-M2-M3-') = false then
  begin
  ...
  begin
    ...
    if cod.Text = '0' then
      descr := 'CRIADO POR: ' + form22.codusario + '-' + form22.usuario
    else
...
begin
  ...
  begin
    MessageDlg('Usuário Não tem Permissão de Usar Essa Rotina!', mtError,
      [mbOK], 1);
    exit;
  end;

  if not verificaPodeExcluir then
    MSG := 'Este Produto Possui Movimento e Não Pode Ser Excluído! Deseja Excluir Mesmo Assim ?'
  else
    MSG := 'Deseja Excluir o Produto: ?' + #13 + 'Cód: ' + cod.Text + #13 +
      'Nome: ' + nome.Text;
...
procedure TForm9.codKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Key = 120) then
  begin // f10
    if funcoes.FiltraNota(funcoes.notaNFe, funcoes.fornecNFe, true) then
    begin
...
begin
  ...
  begin
    grupo := funcoes.localizar1('Localizar Grupo', 'grupoprod', 'cod,nome',
      'cod', '', 'nome', 'nome', true, false, false, 'cod', TEdit(Sender).Text,
      300, Sender);
    if grupo <> '' then
    begin
  ...
  if (Key = #13) and (TEdit(Sender).Text = '') then
  begin
    TEdit(Sender).Text := funcoes.localizar('Localizar Grupo', 'grupoprod',
...
begin
  ...
  begin
    ...
    begin
      fornec := funcoes.localizar1('Localizar Fornecedor', 'fornecedor',
        'cod,nome', 'cod', '', 'nome', 'nome', true, false, false, 'cod',
        TEdit(Sender).Text, 300, Sender);
      if fornec <> '' then
        TEdit(Sender).Text := fornec;
    end;

    if (TEdit(Sender).Text = '') then
    begin
      TEdit(Sender).Text := funcoes.localizar('Localizar Fornecedor',
        'fornecedor', 'cod,nome, cnpj, estado', 'cod', '', 'nome', 'nome', true,
        false, false, '', 300, nil);
    end;
  end;
end;
...
begin
  ...
  if (Key = #13) and (TEdit(Sender).Text <> '') then
  begin
    fabric := funcoes.localizar1('Localizar Fabricante', 'fabricante',
      'cod,nome', 'cod', '', 'nome', 'nome', true, false, false, 'cod',
      TEdit(Sender).Text, 300, Sender);
    if fabric <> '' then
    begin
  ...
  if (Key = #13) and (TEdit(Sender).Text = '') then
  begin
    TEdit(Sender).Text := funcoes.localizar('Localizar Fabricante',
      'fabricante', 'cod,nome', 'cod', '', 'nome', 'nome', true, false, false,
      '', 300, Sender);
  end;
end;
...
begin
  if (Key = #13) and (TEdit(Sender).Text = '') then
  begin
    TEdit(Sender).Text := funcoes.localizar('Localizar Aliquota', 'aliq',
      'cod,aliq,reducao,cst, CSOSN', 'cod', '', 'cod', 'cod', false, false,
      false, '', 430, Sender);
    if TEdit(Sender).Text = '' then
      Key := #0;
  end;
...
begin
  ...
  begin
    ...
    cod.Text := funcoes.localizar1('Localizar Produto', 'produto',
      'cod, nome,quant, p_venda as preco ', 'cod', '', 'nome', 'nome', false,
...
begin
  ...
  begin
    if TEdit(Sender).Text <> '' then
    begin
      if ((funcoes.buscaParamGeral(5, 'N') = 'S') and
        (funcoes.buscaParamGeral(82, 'N') <> 'S')) then
...
begin
  if Key = #13 then
  begin
    CALCLUCRO1(lucro, p_venda, debicm, basedeb, credicm, p_compra, frete,
      encargos, basecred, agregado, ICMS_SUBS.getValor,
      DESC_COMP.getValor, Key);
  end;
end;
...
begin
  ...
  begin
    ...
    Edit1.Text := FormatCurr('#,###,###0.00', CALCPRE(lucro, p_venda, debicm,
      basedeb, credicm, p_compra, frete, encargos, basecred, agregado,
      ICMS_SUBS.getValor, DESC_COMP.getValor));
  end;
end;
...
begin
  ...
  if Key = 118 then
  begin
    cod1 := funcoes.dialogo('normal', 3, '', 60, true, '', 'ControlW',
    ...
    if ret = '' then
    begin
      if addCodBarStringList(cod.Text, cod1) then
        ShowMessage(cod1 + ' Adicionado com sucesso');
    end
    else
    begin
      ShowMessage(ret);
...
begin
  ...
  Label37.Caption := 'F6 - Consulta Cód. Barras' + #13 +
    'F7 - Adicionar Cód. Barras'
end;
...
begin
  ...
  begin
    if Trim(TEdit(Sender).Text) = '' then
      COD_ISPIS.Text := funcoes.VE_CODISPIS('', is_pis.Text);
    if Contido('|' + TEdit(Sender).Text + '|',
      '|100|200|300|400|500|600|700|800|900|') then
    begin
      MessageDlg('Código ' + TEdit(Sender).Text + ' Inválido!', mtInformation,
        [mbOK], 1);
      TEdit(Sender).Text := '';
      Key := #0;
    end;
...
begin
  ...
  begin
    ...
    if not funcoes.Contido('-' + TEdit(Sender).Text + '-',
      '-00-01-02-03-04-05-06-07-08-09-10-99-') then
      TEdit(Sender).Text := '';
  end;
end;
...
begin
  ...
  if cod1 = 0 then
  begin
    MessageDlg('Produto Cód: ' + IntToStr(cod1) + ' é Inválido!', mtInformation,
      [mbOK], 1);
    exit;
  end;
  ...
  begin
    MessageDlg
      ('O sistema Encontrou Uma Inconsistência, O sistema precisa ser Reiniciado!',
      mtInformation, [mbOK], 1);
    funcoes.atualizaBD;
  ...
  dm.IBQuery1.SQL.Add
    ('update or insert into PRODUTO_DELETED(cod,nome,unid,igual,codbar,localiza,fornec,fabric,aliquota,refori,classif,aplic,grupo,serie,is_pis,emb,desc_atac,p_venda, p_venda1,p_compra,lucro,quant,deposito,comissao,credicm,basecred,');
  dm.IBQuery1.SQL.Add
    ('debicm,basedeb,frete,encargos,fracao,agregado,sal,sad,sugestao,compra, dev_icm, equiva, usuario, DATA_DELETED)');
  dm.IBQuery1.SQL.Add
  ...
  dm.IBQuery1.ParamByName('basedeb').AsCurrency :=
    dm.IBselect.fieldbyname('basedeb').AsCurrency;
  dm.IBQuery1.ParamByName('frete').AsCurrency :=
    dm.IBselect.fieldbyname('frete').AsCurrency;
  dm.IBQuery1.ParamByName('encargos').AsCurrency :=
    dm.IBselect.fieldbyname('encargos').AsCurrency;
  dm.IBQuery1.ParamByName('fracao').AsCurrency :=
    dm.IBselect.fieldbyname('fracao').AsCurrency;
  dm.IBQuery1.ParamByName('agregado').AsCurrency :=
    dm.IBselect.fieldbyname('agregado').AsCurrency;
  dm.IBQuery1.ParamByName('sal').AsCurrency := dm.IBselect.fieldbyname('sal')
    .AsCurrency;
  dm.IBQuery1.ParamByName('sad').AsCurrency := dm.IBselect.fieldbyname('sad')
    .AsCurrency;;
  dm.IBQuery1.ParamByName('sugestao').AsCurrency :=
    dm.IBselect.fieldbyname('sugestao').AsCurrency;
  dm.IBQuery1.ParamByName('compra').AsCurrency :=
    dm.IBselect.fieldbyname('compra').AsCurrency;
  dm.IBQuery1.ParamByName('dev_icm').AsCurrency :=
    dm.IBselect.fieldbyname('dev_icm').AsCurrency;
  dm.IBQuery1.ParamByName('equiva').AsString :=
    dm.IBselect.fieldbyname('equiva').AsString;
  ...
  try
    dm.IBQuery1.Transaction.Commit;
    funcoes.gravaAlteracao('Produto Excluido Cod: ' + IntToStr(cod1) + 'Nome: '
      + nome1, 'PRO');
    JsEdit.LimpaCampos(self.Name);
  except
    on E: Exception do
    begin
      MessageDlg('O produto Não Foi Excluído!' + #13 + E.Message, mtError,
        [mbOK], 1);
    end;
  end;
