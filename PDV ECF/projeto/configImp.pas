...
begin
  ...
  arq.Values['controlePorta']  := IfThen(controlePorta.Checked, 'S', 'N');
  arq.Values['interComandos'] := interComandos.Text;
  if arq.Values['interComandos'] = '' then arq.Values['interComandos'] := '100';
...
begin
  ...
  if arq.Values['porta'] <> '' then
   begin
     ...
     if arq.Values['controlePorta'] = 'S' then controlePorta.Checked := true
       else controlePorta.Checked := false;

     interComandos.Text := arq.Values['interComandos'];
     if interComandos.Text = '' then interComandos.Text := '100';
  end;
end;
