...
begin
  ...
  estoqueZero := funcoes.dialogo('not', 0, 'SN', 20, true, 'S', Application.Title, 'Imprimir Itens Com Estoque Zero (S-Sim, N-Não) ?', 'S');
...
begin
 B:=52;
 fornec := funcoes.dialogo('generico',100,'1234567890'+#8,100,false,'','Control For Windows','Informe o Código do Fornecedor:','') ;
...
begin
  ...
  else funcoes.informacao(0, fi, 'Aguarde, Gerando Relatório...', true, false, 5);
...
begin
  h1 := ''; h2 := '';
  grupo := funcoes.dialogo('generico',100,'1234567890'+#8,100,false,'','Control For Windows','Se Deseja Separação por GRUPO, Informe um Cód.:','') ;
  if grupo = '*' then exit;

  if grupo <> '' then h1 := ' and (grupo = '+StrNum(grupo)+')';

  fornec := funcoes.dialogo('generico',100,'1234567890'+#8,100,false,'','Control For Windows','Se Deseja Separação por FORNECEDOR, Informe um Cód.:','') ;
  if fornec = '*' then exit;
...
begin
  ...
  if fabric <> '*' then sql := funcoes.dialogo('generico',0,'123456',0,false,'S','Control For Windows','Qual a Ordem (1-Alfabética, 2-Fornecedor, 3-Fabricante, 4-Numérica, 5-Codbarras, 6-Ref. Original) ?','1');
  if sql = '*' then exit;

  ini := funcoes.dialogo('normal',0,'1234567890'+#8,40,false,'','Control For Windows','Qual o Código Inicial?','');
  if ini = '*' then exit;

  fim := funcoes.dialogo('normal',0,'1234567890'+#8,40,false,'','Control For Windows','Qual o Código Final?','');
  if fim = '*' then exit;

  SelecaoStr := funcoes.dialogo('generico',0,'123',0,false,'S','Control For Windows','Qual o Critério de Seleção?(1-Loja+Depósito 2-Loja 3-Depósito)','1');
...
begin
  ...
  form40.ListBox1.Items.Add('43=Usar Campo Preço de Custo na Consulta de Produtos? (S-Sim N-Não C-Somente Usuarios sem Bloqueios)');
  ...
  form40.ListBox1.Items.Add('79=Qual o Limite de dias para Emissao de NFCe em Contingência ?');
  ...
  form40.ListBox1.Items.Add('84=Qual o fator multiplicativo máximo para conversão de unidades de medida? (Padrao 144)');
  form40.ListBox1.Items.Add('85=Enviar Email Automatico com NFe para o Destinatario ?');
  form40.ListBox1.Items.Add('86=Preencher CFOPs de Substituição Tributaria Automaticamente ?');
  ...
  form40.ListBox1.Items.Add('95=Usar Campo de Informações adicionais de produto no campo OBS do cadastro de Produto ?');
...
begin
  ...
  if form34.juros < 0 then begin
    MessageDlg('Valor de Juros nao pode ser Negativo, Acesso Negado!', mtInformation, [mbOK], 1);
    exit;
  end;
...
begin
  ...
  CODCLI := funcoes.dialogo('generico',0,'1234567890,.'+#8,50,false,'',Application.Title,'Qual o Cód do Cliente?','');
  if CODCLI = '*' then exit;
...
if funcoes.lista1 = '0' then
 ...
 begin
   ...
   form39.ListBox1.Clear;
   form39.conf := 2;
   form39.ListBox1.Items.Add('0=Qual a Nota de Venda(T-Ticket,M-Média,G-Grande,L-Localiza,A-Atacado,R-Resumo,D-Dav,F-Ticket Gráfico, X-Media G., E-Ticket Maior)?');
...
begin
  ...
  sepUsu := funcoes.dialogo('generico',0,'SN',20,false,'S',Application.Title,'Deseja Separar as Vendas por Usuário ?','N');
  if sepUsu = '*' then exit;

  codigoUsu := '';
  h4        := '';
  if sepUsu = 'S' then begin
    codigoUsu := funcoes.dialogo('generico',30,'1234567890' + #8,30,false,'',Application.Title,'Qual o Código do Usuário ?', form22.codusario);
  ...
  h3           := '';
  tipoRegCaixa := 'S';
  tipoRegCaixa := funcoes.dialogo('generico',0,'SN',20,false,'S',Application.Title,'Incluir Lançamentos em Caixa no Resumo de Forma de Pagamento ?', tipoRegCaixa);
...
begin
  ...
  dm.ACBrMail1.Send(false);
    ...
    on e :exception do begin
     ...
     pergunta1.Free;
     ShowMessage('Não Foi Possível Enviar o Email. Verifique se a Conexão com a Internet está Funcionando Corretamente' + #13 + e.Message);
...
begin
   ...
   while true do
     begin
       saidaJustificativa   := funcoes.dialogo('normal', 0, '',300, false,'','Justificativa de Saída','Qual a Justificativa?', saidaJustificativa); //funcoes.MensagemTextoInput('Justificativa De Saída', '');
...
var
  serie1 : String;
begin
  nota := funcoes.dialogo('not',200,'1234567890'+#8+#32,200,true,'',Application.Title,'Qual o Número da Venda ?','');
  if nota = '*' then exit;
...
begin
  try
    funcoes.Mensagem(Application.Title ,'Aguarde, Verificando Estado do Serviço...',15,'Courier New',false,0,clred, false);
    Application.ProcessMessages;
    Application.ProcessMessages;
    Verifica_Status_NFe();
  finally
...
begin
  ...
  grupo := funcoes.dialogo('generico',30,'1234567890' + #8,20,false,'',Application.Title,'Qual o Código do Grupo ?', '');
  if prodzero = '*' then exit;
...
begin
  ...
  ShowMessage('Total de Cupons Não Enviados: ' + IntToStr(dm.IBselect.RecordCount) + #13 +
